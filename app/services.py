...
def build_custom_route(request: CustomRouteRequest) -> CustomRouteResponse:
    ...
    if not selected_modules:
        raise HTTPException(status_code=400, detail="No modules fit your budget and flight duration")

    recommendation = (
        f"{request.traveler_name}, your route includes {len(selected_modules)} mission modules "
        f"for {total_days} days with total budget {total_price:.2f} million credits."
    )
...
def update_booking_status(booking_id: str, status: BookingStatus) -> Booking:
    ...
    if booking.status == BookingStatus.canceled:
        raise HTTPException(status_code=409, detail="Canceled booking cannot be changed")
