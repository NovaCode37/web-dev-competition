...
def list_tours(max_price_mln: float | None, max_duration_days: int | None, difficulty: Difficulty | None, search: str | None) -> TourListResponse:
    ...
    if search:
        ...
        tours = [tour for tour in tours if query in tour.title.lower() or query in tour.destination.lower() or query in tour.description.lower()]
...
def build_custom_route(request: CustomRouteRequest) -> CustomRouteResponse:
    ...
    recommendation = f"{request.traveler_name}, маршрут включает {len(selected_modules)} модулей на {total_days} дней с бюджетом {total_price:.2f} млн USD."
    return CustomRouteResponse(traveler_name=request.traveler_name, route_modules=selected_modules, total_days=total_days, total_price_mln=round(total_price, 2), recommendation=recommendation)
...
def create_booking(request: BookingRequest) -> Booking:
    ...
    booking = Booking(id=uuid4().hex[:12], tour_id=request.tour_id, full_name=request.full_name, email=request.email, travelers_count=request.travelers_count, total_price_mln=round(tour.price_mln * request.travelers_count, 2), status=BookingStatus.pending_confirmation, created_at=datetime.now(UTC), special_request=request.special_request)
...
def get_stats() -> AgencyStats:
    ...
    return AgencyStats(tours_total=len(TOURS), active_bookings=sum(1 for booking in BOOKINGS.values() if booking.status != BookingStatus.canceled), available_seats=available_seats, destinations=destinations)
